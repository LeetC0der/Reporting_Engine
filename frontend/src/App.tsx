...
type NavSection = 'pipeline' | 'connections' | 'settings';
...
export default function App() {
  ...
  const [selectedSection, setSelectedSection] = useState<NavSection>('pipeline');
  ...
  useEffect(() => {
    ...
    const navState = states.find((item) => item.componentKey === 'active-nav');
    ...
    const themeState = states.find((item) => item.componentKey === 'theme-preference');

    if (navState) {
      setSelectedSection(JSON.parse(navState.stateValue).section);
    }
    if (toggleState) {
    ...
    if (themeState) {
      const nextTheme = JSON.parse(themeState.stateValue).mode as ThemeMode;
  ...
  const selectSection = (section: NavSection): void => {
    setSelectedSection(section);
    saveUiMutation.mutate({
      componentKey: 'active-nav',
      stateType: 'navigation',
      stateValue: JSON.stringify({ section })
    });
  };

  return (
    <MantineProvider forceColorScheme={themeMode} defaultColorScheme="light">
      {!authed ? (
        ...
        >
          <AppShell.Navbar p="md">
            <Stack>
              <Title order={4}>Reporting Engine</Title>
              <NavLink
                active={selectedSection === 'pipeline'}
                label="Pipeline"
                ...
                <Card withBorder>
                  <Stack>
                    ...
                    <Group>
                      <Text>Theme</Text>
                      <Button
                        variant={themeMode === 'light' ? 'filled' : 'light'}
                        onClick={() => {
                          setThemeMode('light');
                          saveUiMutation.mutate({
                            componentKey: 'theme-preference',
                            stateType: 'form',
                            stateValue: JSON.stringify({ mode: 'light' })
                          });
                        }}
                      >
                        Light
                      ...
                      <Button
                        variant={themeMode === 'dark' ? 'filled' : 'light'}
                        onClick={() => {
                          setThemeMode('dark');
                          saveUiMutation.mutate({
                            componentKey: 'theme-preference',
                            stateType: 'form',
                            stateValue: JSON.stringify({ mode: 'dark' })
                          });
                        }}
                      >
                        Dark
